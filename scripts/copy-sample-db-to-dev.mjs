...
const copySampleDatatoDevDb = async () => {
    ...
    const files = fs.readdirSync(path.join(process.cwd(),'.sample-database-data')).filter(x=>x.endsWith('.json'));

    for (const file of files) {
        console.log('processing', file);
        const collection = db.collection(file.substring(file.lastIndexOf('/')+1).replace('.json', ''));
