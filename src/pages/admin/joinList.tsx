...
export const JoinList = () => {
  ...
  useEffect(() => {
    if (session) {
      if (joinInfo.error) {
        if (joinInfo.error.response.statusText.includes('not authorized')) {
          router.replace('/auth/not-authorized');
        }
        enqueueSnackbar(
          `Failed to retrieve list. ${joinInfo.error.response.statusText}`,
          {
            variant: 'error',
          }
        );
  ...
  }, [router, joinInfo.joiners, joinInfo.error, enqueueSnackbar, session]);
      ...
      <PrimaryLayout>
        <Container maxWidth="xs">
          <Paper className={classes.paper}>
            ...
            <Link href="/auth/signIn?redirect=admin_joinList" passHref>
